...
class Proteins():
    ...
    def form_seq_coords(self, chain_dict):
        ...
        for residue in residues:
            ...
            res_dict[index] = {}
            res_dict[index]['aa_one'] = seq1(residue.get_resname(), custom_map={"UNK": "X"})

            b_factor = 0.0
            if 'CA' in residue:
                b_factor = residue['CA'].get_bfactor()
            elif len(residue.get_list()) > 0:
                b_factor = residue.get_list()[0].get_bfactor()
            res_dict[index]['b_factor'] = round(b_factor, 3)
        ...
        for index in seq_index:
            if index in res_dict: 
                ...
                b_factors.append(res_dict[index]['b_factor'])
            else: 
                chain_seq += '-'
                for key in coord_dict: coord_dict[key].append([0,0,0])
                b_factors.append(0.0)
        
        return chain_seq, coord_dict, b_factors, residues_icode
    ...
    def make_protein(self, model, chains_info):
        ...
        for chain in protein:
            ...
            protein[chain]['sequence'], protein[chain]['coords'], protein[chain]['b_factors'], residues_icode = self.form_seq_coords(protein[chain])
            if residues_icode != []: protein[chain]['residues_icode'] = self.form_resicode_dict(residues_icode)

        # Keep residues for structural context calculations later
        # for chain in protein: 
        #     if chain == 'flag': continue
    ...
    def get_structural_context(self, chain_id, pos, chains_info):
        if chain_id not in self.complex:
            return None
            
        ab_chains = str(chains_info.split('_')[0])
        ag_chains = str(chains_info.split('_')[1])
        
        is_ab = chain_id in ab_chains
        target_chains = ag_chains if is_ab else ab_chains
        
        chain_data = self.complex[chain_id]
        if pos not in chain_data['seq_index']:
            return None
            
        idx = chain_data['seq_index'].index(pos)
        
        # 1. Interface status
        # In make_protein, mask was incremented (+1) if it was in the interface.
        # But wait, trim_proteins modifies mask. The simplest way is to check the mask value.
        # If it's an interface residue, it was +1. But form_mut_chain might add +2.
        # We can just check distance manually.
        ...
        for res in residues:
            if res.get_id()[1] == pos:
                for atom in res:
                    mut_coords.append(atom.coord)
                break
                
        if not mut_coords:
            return None
            
        # Get target chains coordinates
        target_coords = []
        for c in target_chains:
            if c in self.complex:
                c_residues = self.complex[c].get('residues', [])
                for res in c_residues:
                    for atom in res:
                        target_coords.append(atom.coord)
                        
        # 2. Distance to antigen
        min_dist = float('inf')
        if target_coords:
            mut_coords = np.array(mut_coords)
            target_coords = np.array(target_coords)
            # pairwise distances
            for mc in mut_coords:
                dists = np.linalg.norm(target_coords - mc, axis=1)
                min_dist = min(min_dist, np.min(dists))
                
        is_interface = min_dist <= self.radius
        
        # 3. Neighborhood
        seq = chain_data['sequence']
        neighborhood = []
        for i in range(max(0, idx-2), min(len(seq), idx+3)):
            n_pos = chain_data['seq_index'][i]
            n_res = seq[i]
            # e.g. VH48
            neighborhood.append(f"{chain_id}{n_pos}")
            
        return {
            'distance_to_antigen': round(float(min_dist), 2) if min_dist != float('inf') else None,
            'is_interface': bool(is_interface),
...
def form_mut_chain(chain,  mutation_chain):
    ...
    for i, mut in enumerate(mut_index):
        ...
        if sequence[index] == ori_aa[i]:
            ...
            raise MutationMismatchError(f"Expected '{ori_aa[i]}' at pos {mut} but found '{sequence[index]}' in structure.")

    return ori_sequence, mask, sequence, coords, b_factors
...
def get_chains_dict(mutation_info, strct):
    ...
    for chain in strct:
        ...
        if chain in cplx_muts:
            pair['ori_seq'], pair['mask'], pair['mut_seq'], pair['coords'], pair['b_factors'] = form_mut_chain(strct[chain], cplx_muts[chain])
        else:
            pair['ori_seq'], pair['mask'], pair['mut_seq'], pair['coords'], pair['b_factors'] = strct[chain]['sequence'], strct[chain]['mask'], [], strct[chain]['coords'], strct[chain]['b_factors']
        pairs.append(pair)
...
def completize(protein, esm = None):
    ...
    B = np.zeros([chain_num, l_max], dtype=np.float32)
    if esm == None:
    ...
    for i, chain in enumerate(protein):
        ...
        if esm == None:
            ...
            # Map amino acids directly to their indices in esm2 alphabet, mapping '-' to 30
            esm_map = {'<cls>': 0, '<pad>': 1, '<eos>': 2, '<unk>': 3, 'L': 4, 'A': 5, 'G': 6, 'V': 7, 'S': 8, 'E': 9, 'R': 10, 'T': 11, 'I': 12, 'D': 13, 'P': 14, 'K': 15, 'Q': 16, 'N': 17, 'F': 18, 'Y': 19, 'M': 20, 'H': 21, 'W': 22, 'C': 23, 'X': 24, 'B': 25, 'U': 26, 'Z': 27, 'O': 28, '.': 29, '-': 30, '<null_1>': 31, '<mask>': 32}
            oS[i,1:N+1] = [esm_map.get(a, 3) for a in chain['ori_seq']] # default to <unk>
            oS[i,N+1] = 2
            if chain['mut_seq'] == []: mS[i,1:N+2] = copy.deepcopy(oS[i,1:N+2])
            else: 
                mS[i,1:N+1] = [esm_map.get(a, 3) for a in chain['mut_seq']]
                mS[i,N+1] = 2
            # print('oS/mS:', oS.shape, mS.shape)

    X = torch.from_numpy(X).float()        # [C, L_max, 4, 3]
    oS = torch.from_numpy(oS).long()       # [C, L_max]
    mS = torch.from_numpy(mS).long()       # [C, L_max]
    L = torch.from_numpy(L).float()      # [C, L_max]
    M = torch.from_numpy(M).float()      # [C, L_max]
    B = torch.from_numpy(B).float()      # [C, L_max]

    return [X.unsqueeze(0), oS.unsqueeze(0), mS.unsqueeze(0), L.unsqueeze(0), M.unsqueeze(0), B.unsqueeze(0)]
