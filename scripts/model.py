...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, E_idx, mask):
        ...
        ii = torch.arange(N_nodes, dtype=torch.float32, device=E_idx.device).view((1, -1, 1))     # [1,C*N',1]
...
class ComplexFeatures(nn.Module):
    ...
    def _dist(self, X, mask, eps=1E-6):
        ...
        mask_2D = mask_2D - torch.eye(N).unsqueeze(0).to(X.device)
    ...
    def _orientations_coarse(self, X, mask, E_idx):
        ...
        n_2 = F.normalize(torch.cross(u_2, u_1, dim=-1), dim=-1)    # [B*C, N'-3, 3]
    ...
    def forward(self, X, mask, B_factor=None):
        # X [B, C, N, 4, 3], mask [B, C, N]
        B, N = X.size(0), X.size(2)
        ...
        if B_factor is not None:
            V = torch.cat((V, B_factor.view(B, -1, 1)), dim=-1)
        # edge features    
...
class Decoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.esm != None:
            ...
            self.model_esm, self.alphabet = esm.pretrained.esm2_t33_650M_UR50D()
            self.batch_converter = self.alphabet.get_batch_converter()
            self.model_esm.eval()
            self.W_esm = nn.Sequential(
                    nn.Linear(1280, self.hidden_size),
                    nn.ReLU(),
    ...
    def make_X_blocks(self, x, l, r, mask, M=False):
        ...
        if not M:
            ...
            middle = x[l:r+1,:] if x.dim() > 1 else x[l:r+1]
            bX = torch.cat(lblocks + [middle] + rblocks, dim=0)
        else:
    ...
    def make_chain_blocks(self, x, os, ms, l, mask, b):
        # x [N,4,3]
        # os, ms [N,hidden_size]
        # l, mask [N]
        if torch.all(l == 0).item():
            ...
            hb = torch.cat([self.mask_mean(b, mask, i) for i in range(0, end, self.block_size)])
    ...
    def make_proteins(self, X, oS, mS, L, mask, B_factor):
        ...
        if self.esm == None:
            ...
            repre_oS = torch.zeros([oS.size(0)*oS.size(1), oS.size(2), 1280], dtype=torch.float32)
            repre_mS = torch.zeros([mS.size(0)*mS.size(1), mS.size(2), 1280], dtype=torch.float32)
            for b in range(repre_oS.size(0)):
                with torch.no_grad():
                    torch.cuda.empty_cache()
                    repre_oS[b,:] = self.model_esm(oS.view(-1,oS.size(2))[b,:].unsqueeze(0), repr_layers=[33], return_contacts=False)["representations"][33]
                    repre_mS[b,:] = self.model_esm(mS.view(-1,mS.size(2))[b,:].unsqueeze(0), repr_layers=[33], return_contacts=False)["representations"][33]

            hoS = torch.zeros([seq_lens.size(0), mask.size(-1), self.hidden_size], dtype=torch.float32).to(X.device)
            hmS = torch.zeros([seq_lens.size(0), mask.size(-1), self.hidden_size], dtype=torch.float32).to(X.device)
            for i, chain_len in enumerate(seq_lens):
                if chain_len == 1: continue
                hoS[i, : chain_len - 2] = self.W_esm(repre_oS[i, 1:chain_len-1].to(X.device))
                hmS[i, : chain_len - 2] = self.W_esm(repre_mS[i, 1:chain_len-1].to(X.device))

        # pad
        bX = torch.zeros(B*C,self.L_max,4,3).to(X.device)
        boS = torch.zeros(B*C,self.L_max,self.hidden_size).to(X.device)
        bmS = torch.zeros(B*C,self.L_max,self.hidden_size).to(X.device)
        bL = torch.zeros(B*C,self.L_max).to(X.device)
        bmask = torch.zeros(B*C,self.L_max).to(X.device)
        bB = torch.zeros(B*C,self.L_max).to(X.device)

        for i in range(B*C):
            if not torch.all( mask[i,:] == 0.0).item():
                b_i = B_factor[i,:] if B_factor is not None else None
                results, n = self.make_chain_blocks(X[i,:], hoS[i,:], hmS[i,:], L[i,:], mask[i,:], b_i)
                bX[i,:n,:], boS[i,:n,:], bmS[i,:n,:], bL[i,:n], bmask[i,:n], bB[i,:n] = results
    ...
    def forward(self, X, oS, mS, L, mask, B_factor=None, ddg_true=None):

        # X [B,C,L,4,3], oS/mS [B,C,L,hidden_size], L/mask [B,C,L]
        has_B_factor = B_factor is not None
        # coarse-granulated and apliced all chains into a compete complex tensor
        X, oS,  mS, L, mask, B_factor = self.make_proteins(X, oS, mS, L, mask, B_factor)    # X [B,C,N,4,3], oS [B,C*N,hidden_size], mask [B,C,N]
        if not has_B_factor:
            B_factor = None
        B = mask.size(0)
        # feature extraction
        V, E, E_idx = self.features(X, mask, B_factor)      # V: [B,C*N,6 or 7]   E: [B,C*N,K,39]   E_idx: [B,C*N,K]
        ...
        # predicting
        feat_wm = torch.cat([h_ori, h_mut], dim=-1).to(X.device)  # [B, C*N', hidden_size*2]
        feat_mw = torch.cat([h_mut, h_ori], dim=-1).to(X.device)
